...
if ($_SERVER["REQUEST_METHOD"] != "PUT"):
  ...
  if (empty($data) || !isset($data->mobile) || empty(trim($data->mobile))):
    ...
    try {
      $update = "UPDATE `users` SET `ipaddress`=:ipaddress WHERE `mobile`=:mobile";
      $update_stmt = $conn->prepare($update);
      $update_stmt->bindValue(':mobile', $data->mobile, PDO::PARAM_INT);
      $update_stmt->bindValue(':ipaddress', $data->ipaddress, PDO::PARAM_STR);
      if ($update_stmt->execute()):
        $returnData = $error_handler->getResponse(1, 200, 'IP Address updated successfully!');
      else:
